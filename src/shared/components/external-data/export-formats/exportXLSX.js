...
/**
 * @brief Function for export in XLSX (Excel Default)
 * 
 ...
 * 
 * behavior:
 * - Saves the dataset as an XLSX file named `${baseName}.xlsx`.
 * - If `includeInterpPoints` is true, includes interpolation data points in the export.
 */
export const exportXLSX = (ds, baseName, includeInterpPoints) => {
  ...
  if(ds.type === "CVW" || ds.type === "DPV" || ds.type === "SWV"){
    ...
    rowPtr++
    merges.push({ s: { r: rowPtr-1, c: 3 }, e: { r: rowPtr-1, c: 4 } })
    ws[`D${rowPtr}`] = { v: "Advanced Parameters" }
    rowPtr++
    Object.entries(ds.params_e || {}).forEach(([k, v]) => {
      ws[`D${rowPtr}`] = { v: k }
      ws[`E${rowPtr}`] = { v: v }
      rowPtr++
    })
    ...
    interps.forEach((interp, idx) => {
      ...
      const colX = XLSX.utils.encode_col(startCol)
      const colY = XLSX.utils.encode_col(startCol + 1)
      let logConst = interp.type === 'logspline' ? 4 : 0
      ...
      const title =
        interp.type === 'polinomial'
          ...
          : interp.type === 'gaussiana'
            ...
            : interp.type === 'logspline'
              ? `LogSpline (${interp.typeCalculate}) - ${interp.ref}`
              : interp.type || 'Interp'
      ...
      if (interp.type === 'polinomial' && Array.isArray(interp.coefficients)) 
        ...
        pStr = `σ: ${interp.sigma} μ: ${interp.mu} A: ${interp.amplitude}`
      else if (interp.type === 'logspline' && interp.coefficients) {
        const { a = [], b = [], c = [], d = [] } = interp.coefficients
        const fmt = arr => `[${arr.map(v => Number(v).toPrecision(1)).join(', ')}]`
        pStr = [`a: ${fmt(a)}`, `b: ${fmt(b)}`, `c: ${fmt(c)}`, `d: ${fmt(d)}`]
      }

      if (interp.type === 'logspline' && interp.coefficients) {
        const { a = [], b = [], c = [], d = [] } = interp.coefficients
        ...
        paramLines.forEach((line, i) => {
          ws[`${colX}${2 + i}`] = { v: line, s: { alignment: { horizontal: 'left' } } }
        })
      }
